...
<div class="card table-card">
  <?php if (!$branches): ?>
    ...
    <p>管理者DBの <code>user_locations</code>、<code>locations</code>、<code>tenant_db_connections</code>、<code>tenant_db_pool</code> または <code>admin_branch_db_assignments</code> を確認してください。</p>
  <?php else: ?>
    <form method="post">
      <?= Csrf::field() ?>
      <table class="data-table">
        <thead><tr><th>拠点</th><th>権限</th><th>DB名</th><th>DB末尾</th><th></th></tr></thead>
        <tbody>
        <?php foreach ($branches as $branch): ?>
          <tr>
            <td><?= h((string)$branch['branch_name']) ?><br><small><?= h((string)$branch['branch_uid']) ?> / <?= h((string)($branch['branch_code'] ?? '')) ?></small></td>
            <td><?= h((string)($branch['role_at_location'] ?? '-')) ?></td>
            <td><?= h((string)($branch['branch_db_name'] ?? '未割当')) ?></td>
            <td><?= h((string)($branch['branch_db_suffix'] ?? '-')) ?></td>
